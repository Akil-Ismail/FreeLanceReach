...
export default function HomeMatchingPage() {
  ...
  // Sort: pending action first, then by score desc
    ...
    >
      ...
      <div className="space-y-3">
        <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide">
          Match Results {!loading && `(${matches.length})`}
        </h2>
        ...
        {loading ? (
          ...
          sorted.map((match) => {
            ...
            const meta       = STATUS_STYLE[match.status] || "bg-gray-100 text-gray-500";
            ...
            return (
              <div
                ...
                className={`bg-white border rounded-2xl p-5 shadow-sm transition ${
                  actionable ? "border-gray-200 hover:border-gray-300" : "border-gray-100 opacity-75"
                }`}
              >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-0.5">
                      ...
                      {match.freelancer?.freelance_category && (
                        <span className="text-xs text-gray-400">{match.freelancer.freelance_category}</span>
                      )}
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${meta}`}>
                        {match.status.replace(/_/g, " ")}
                      </span>
                    </div>
                    <p className="text-xs text-gray-400">
                      {match.proposal?.title || `Proposal #${match.proposal_id}`}
