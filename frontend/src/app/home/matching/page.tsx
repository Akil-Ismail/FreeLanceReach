...
import { CheckCircle2, XCircle, RefreshCw, FileText, User, Briefcase, ChevronDown } from "lucide-react";
...
export default function HomeMatchingPage() {
  ...
  const rerunAndRefresh = useCallback(async () => {
    ...
    try {
      if (role === "company") {
        const proposalsRes = await api.get("/proposals", { params: { actor_user_id: userId } });
        await Promise.all(
          (proposalsRes.data || []).map((p: { id: number }) =>
            api.post(`/proposals/${p.id}/match`, { actor_user_id: userId }).catch(() => {})
          )
        );
      }
      const res = await api.get("/matches", { params: { actor_user_id: userId } });
  ...
  const respond = async (matchId: number, approve: boolean) => {
    ...
    try {
      await api.post(`/matches/${matchId}/respond`, { actor_user_id: userId, approve });
  ...
  const removeApproval = async (matchId: number) => {
    setRespondingId(matchId);
    try {
      await api.post(`/matches/${matchId}/respond`, { actor_user_id: userId, approve: true, remove_approval: true });
  ...
  const needsAction = (m: Match) => {
    if (role === "company")   return m.status === "pending" || m.status === "freelancer_approved";
    if (role === "freelancer") return m.status === "pending" || m.status === "company_approved";
    return false;
  };

  const canRemoveApproval = (m: Match) => {
    if (role === "company")   return m.status === "company_approved" || m.status === "mutual_approved";
    if (role === "freelancer") return m.status === "freelancer_approved" || m.status === "mutual_approved";
    return false;
  };
  ...
  // Group by proposal
  const groups = visible.reduce<Record<string, { title: string; items: Match[] }>>((acc, m) => {
  ...
  return (
    <SectionShell
      ...
      actions={
        <div className="flex items-center gap-2">
          <SearchBar value={query} onChange={setQuery} placeholder="Search freelancer or proposal…" />
          <button
    ...
    >
      ...
      {loading ? (
        <div className="bg-white border border-gray-200 rounded-2xl p-8 text-center text-gray-400 text-sm">Loading matches…</div>
                    ...
                    <li key={match.id} className={`px-5 py-4 ${actionable ? "" : "opacity-70"}`}>
                      <div className="flex items-center justify-between gap-4 flex-wrap">
                        ...
                        <div className="flex items-center gap-3 flex-wrap">
                          ...
                          <button
                            onClick={() => router.push(`/home/profiles/${match.freelancer_user_id}`)}
